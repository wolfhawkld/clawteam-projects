...
**核心直觉**：Mamba 的线性变换，本质是根据输入语义动态决定"保留什么、丢弃什么"。

#### 语义如何变成控制信号

预训练时，Linear 层学到"语义 → 记忆策略"的映射：
...
| 语义模式 | 学到的行为 | 原因 |
|----------|-----------|------|
| 句子结束标点 | Δ 变大，快速重置状态 | 准备接收新句子 |
| 人名/实体 | B 变大，加强写入 | 重要信息需要记住 |
| 填充词"的、了、着" | B 变小，弱写入 | 语义贡献低 |
| 疑问词"什么、哪里" | C 调整输出焦点 | 准备检索相关内容 |

#### 隐藏状态的语义解释

隐藏状态 $h_t$ 是一个**固定大小的语义容器**（如 16 维向量）。选择性机制就是：

> **根据语义决定：往容器里装多少、倒出什么、清空多少**
...
#### 与 Transformer 的语义处理对比

| 方式 | 语义匹配时机 | 实现"选择性"的方法 |
|------|-------------|-------------------|
| Transformer | 查询时（Query-Key 点积） | 存所有 → 用时查 |
| Mamba | 写入时（Linear 层 → B/C/Δ） | 边读边筛选 → 压缩存储 |

**Transformer**：语义匹配发生在注意力计算时

**Mamba**：语义判断发生在信息写入时——Linear 层本质上是一个"语义评估器"

#### 线性变换的语义作用

$h_t = \bar{A}h_{t-1} + \bar{B}x_t$ 中的线性变换：

- **不是**无意义的矩阵乘法
- **而是**"语义压缩器"：无关信息被压缩/丢弃，重要信息被保留/加强
- **依据**就是输入的语义——Linear 层的权重编码了"什么语义重要、如何处理"

这就是选择性机制的数学本质：**从固定线性变换，变成内容自适应的语义压缩变换**。
