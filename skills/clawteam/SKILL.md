...
当用 ClawTeam 做研究任务时，spawn 的 agent **不继承 skills**。需要在 task prompt 里嵌入搜索方法指导。

以下模板可直接嵌入 task prompt：
...
curl -s "https://api.tavily.com/search" \
  ...
  -d '{"query": "搜索关键词", "include_answer": true, "max_results": 5}' | jq .

参数：
- query: 搜索内容
- include_answer: true/false（是否生成 AI 总结）
- max_results: 结果数量（1-10）
- include_domains: 限定域名列表，如 ["arxiv.org", "nature.com"]
- days: 限定最近 N 天
```
...
```
使用 OpenAlex API 搜索学术论文（免费，无需 API key）：

# 论文搜索
curl -s "https://api.openalex.org/works?search=mamba+memory&per_page=10" | jq '.results[] | {title, doi, cited_by_count, publication_year}'

# 过滤筛选
curl -s "https://api.openalex.org/works?search=LLM&filter=publication_year:2024,is_oa:true&per_page=5" | jq '.results[] | {title, doi, cited_by_count}'

# 作者搜索
curl -s "https://api.openalex.org/authors?search=Albert+Gu" | jq '.results[] | {display_name, works_count, cited_by_count}'

# 引用追踪
curl -s "https://api.openalex.org/works?filter=cites:https://doi.org/10.48550/arxiv.2312.00752" | jq '.results[] | {title, year: .publication_year}'
```

### 网页内容提取

```
使用 web_extract 工具提取网页内容（支持 PDF/arxiv）：

from hermes_tools import web_extract
result = web_extract(["https://arxiv.org/abs/2312.00752"])
# 返回 markdown 格式的论文内容
```
...
```bash
clawteam spawn -t research-team -n surveyor --task "
调研「Mamba 用于 Agent Memory」相关研究。

搜索方法：
1. 学术搜索：curl -s 'https://api.openalex.org/works?search=mamba+agent+memory&per_page=10' | jq '.results[] | {title, doi, cited_by_count}'
2. 网络搜索：curl -s 'https://api.tavily.com/search' -H 'Authorization: Bearer \$TAVILY_API_KEY' -H 'Content-Type: application/json' -d '{\"query\": \"mamba agent memory architecture\", \"include_answer\": true, \"max_results\": 5}' | jq .

任务要求：
- 搜索关键词组合：mamba memory, SSM agent, state space model memory
- 整理发现的论文（标题、年份、引用数、核心方法）
- 分析研究现状与空白领域
- 产出保存到 ~/.openclaw/workspace/clawteam-projects/<项目名>/survey.md
"
```

**关键点**：
- Skills 不继承 → 搜索方法必须显式写在 task prompt 里
- 使用 `\$` 转义环境变量，防止 shell 提前解析
- 指定明确的产出路径和格式要求
